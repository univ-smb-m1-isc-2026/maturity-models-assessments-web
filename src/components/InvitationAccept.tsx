...
const InvitationAccept = () => {
  ...
  const [message, setMessage] = useState(() => {
    if (!currentUser) return "Connectez-vous ou inscrivez-vous pour accepter l'invitation.";
    if (!token) return "Lien d'invitation invalide.";
    return "";
  });
  ...
  useEffect(() => {
    ...
    TeamService.acceptInvitation(token).then(
      (response) => {
        setStatus("success");
        setMessage(response.data.message || "Invitation acceptée.");
      },
      (error) => {
        const resMessage =
        ...
        if (error.response?.status === 401 || error.response?.status === 403) {
          setStatus("need_auth");
          setMessage("Connectez-vous ou inscrivez-vous pour accepter l'invitation.");
        } else {
          setStatus("error");
  ...
  return (
    <div className="max-w-lg mx-auto p-6">
      ...
      <div className={`rounded-md p-4 ${status === "success" ? "bg-green-900/30 border border-green-800" : status === "error" ? "bg-red-900/30 border border-red-800" : "bg-blue-900/30 border border-blue-800"}`}>
        <p className="text-sm">{message || "Traitement en cours..."}</p>
      </div>
      {status === "need_auth" && (
